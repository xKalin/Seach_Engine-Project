			...
			while(scan.hasNext()) { // clear all lines
				...
				if(val.equals(".T") ) {
					...
					while(true){
						...
						//
						//repeat for stememd word if stem is true
						if(stem) {
							if(post.containsKey(stemmedWord)) {
								term word = post.get(stemmedWord);
								if(ID != word.getID(word.getIDListSize()-1))//check if new DocID has been set
									word.createDocID(ID);
								word.setFreq(ID);
								word.setPos(ID,pos);
							} else { //create new obj word
								post.put(stemmedWord, new term(stemmedWord));
								term word = post.get(stemmedWord);
								word.createDocID(ID);
								word.setFreq(ID);
								word.setPos(ID, pos);
							...
							//dictionary
							//check for first occurrence in specific doc (if true skip word)
							if(temp.contains(stemmedWord)) { 
									continue;
							}else {
								temp.add(stemmedWord);
							}
							//if new term add to dictionary and set df to 1 .. else increment df
							if(!df.containsKey(stemmedWord)) {
								dictionary.add(stemmedWord);
								df.put(stemmedWord,1);
							}else {
